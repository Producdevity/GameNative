...
public class NewXConnectorEpoll {
    private final NewConnectionHandler connectionHandler;
    private long nativePtr;
    private final NewRequestHandler requestHandler;
    ...
    static {
        System.loadLibrary("winlatornew");
    }

    public NewXConnectorEpoll(UnixSocketConfig socketConfig, NewConnectionHandler connectionHandler, NewRequestHandler requestHandler) {
        this.connectionHandler = connectionHandler;
        this.requestHandler = requestHandler;
        long jNativeAllocate = nativeAllocate(socketConfig.path);
        this.nativePtr = jNativeAllocate;
        if (jNativeAllocate == 0) {
            throw new RuntimeException("Failed to allocate NewXConnectorEpoll.");
        }
    }
