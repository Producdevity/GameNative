...
public class NewXOutputStream {
    ...
    public NewXOutputStream(int clientFd, int initialCapacity) {
    ...
    private class OutputStreamLock implements XStreamLock {
        public OutputStreamLock() {
            NewXOutputStream.this.lock.lock();
        }

        @Override // com.winlator.xconnector.XStreamLock, java.lang.AutoCloseable
        public void close() throws IOException {
            try {
                if (!NewXOutputStream.sendData(NewXOutputStream.this.nativePtr)) {
                    throw new IOException("Failed to send data.");
                }
            } finally {
                NewXOutputStream.this.lock.unlock();
            }
        }
    }
}
